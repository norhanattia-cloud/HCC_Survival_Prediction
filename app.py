...
raw_data = {
    'Age': age, 'Gender': 1 if gender == "Male" else 0, 'Symptoms': 1 if symptoms == "Yes" else 0,
    'Alcohol': 1 if alcohol == "Yes" else 0, 'Smoking': 1 if smoking == "Yes" else 0,
    'Diabetes': 1 if diabetes == "Yes" else 0, 'Obesity': 1 if obesity == "Yes" else 0,
    'Hepatitis_B': 1 if hepb == "Yes" else 0, 'Hepatitis_C': 1 if hepc == "Yes" else 0,
    'Cirrhosis': 1 if cirrhosis == "Yes" else 0, 'Endemic_Calcification': 1 if endemic == "Yes" else 0,
    'NASH': 1 if fatty_liver == "Yes" else 0, 'Total_Bilirubin': total_bil, 'Direct_Bilirubin': direct_bil,
    'AST': ast, 'ALT': alt, 'ALP': alp, 'AFP': afp, 'Albumin': albumin, 'Hemoglobin': hemoglobin,
    'Platelets': platelets, 'INR': inr, 'Creatinine': creatinine, 'Ferritin': ferritin,
    'Number_of_Nodules': 1 if nodules == "Single" else 2, 'Major_Dimension': major_dim,
    ...
    'Portal_Hypertension': 1 if portal_htn == "Yes" else 0, 'Metastasis': 1 if metastasis == "Yes" else 0,
    'AST_ALT_Ratio': ast_alt_ratio, 'ALBI_Score': albi_score, 'FIB4_Score': fib4_score, 'Child_Pugh_Score': cp_score
}
...
if st.button("Predict Survival", key="predict_btn"):
    ...
    scaler_input = pd.DataFrame(0.0, index=[0], columns=scaler_cols)
    
    
    for col in scaler_cols:
        if col in input_df.columns:
            scaler_input.at[0, col] = input_df.at[0, col]
        else:
            for raw_col in input_df.columns:
                if col.lower().replace(" ", "_") == raw_col.lower().replace(" ", "_"):
                    scaler_input.at[0, col] = input_df.at[0, raw_col]
                    break
                    
    
    scaled_values = scaler.transform(scaler_input)
    scaled_df = pd.DataFrame(scaled_values, columns=scaler_cols)
    
     
    final_input = scaled_df[features]
